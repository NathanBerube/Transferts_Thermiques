...
# Code pour le graphique qui met en relation le diamètre du tuyau et les pertes de chaleur
# pour plusieurs épaisseurs d'isolant
...
# paramètres du tuyau
D_tuy = np.linspace(0.10,0.5,200) # diametre du tuyau d'alimentation
k_tuy = 50 # conductivité du tuyau en acier au carbone, estimée à 50 w/m.K
L_tuy = 1500 # longueur du tuyau, fixée à 1.5 km
t_tuy = 0.01 # épaisseur du tuyau en m
Z_tuyau = 1 # profondeur du tuyau dans le sol en m

# paramètres de l'isolant
D_iso = D_tuy+(2*t_tuy) # diamètre interne de la couche d'isolant en m
t_iso = np.linspace(0,0.10,6) # plage de valeurs pour l'épaisseur d'isolant en m pour construire le graphique
...
for epaisseur_iso in t_iso:
    ...
    plt.plot(D_tuy, np.abs(pertes)/1000, label=f"Épaisseur de l'isolant = {epaisseur_iso*100:.0f} cm")
