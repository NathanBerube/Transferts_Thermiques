...
n_profondeur = 4
Z_tuyau = np.linspace(3*D_tuy/2, 3, n_profondeur)

for profondeur in Z_tuyau:
    # paramètres de l'isolsant
    D_iso = D_tuy+(2*t_tuy)
    t_iso = np.linspace(0,0.05,100)
    L_iso = L_tuy
    k_iso = 0.03 #conductuvité thermique de l'isolant - 0.035 = laine de verre ou laine de roche ou du polyurethane
    ...
    R_S = R_profond(L_tuy, profondeur, D_iso + 2*t_iso, conductivite_sol)
    ...
    plt.plot(t_iso, np.abs(pertes)/1000, label=f"Profondeur = {profondeur:.2f} m")
